...
import { OPPORTUNITY_LIST } from '../tableHeader';
...
export class OpportunityListComponent implements OnInit {
  ...
  constructor(private modalService: NgbModal, private leadRest: LeadService, private common: CommonService,
    private notifier: NotifierService, private route: Router, private config: ConfigService) {
  ...
  ngOnInit(): void {
    ...
    this.getAllOpportunity();
    this.tableHeader = OPPORTUNITY_LIST;
  ...
  getAllOpportunity(): any {
    ...
    this.leadRest.getAllOpportunity(param).subscribe((res: any) => {
      console.log("opp Res>>>>>>>>>>", res);
      // this.common.spinnerHide();
      if ((res.success) && (res.status == 200)) {
        let tempArray = [];
        for (let i = 0; i < res.response.allExitingUser.length; i++) {
          if (res.response.allExitingUser[i].id != null) {
            tempArray.push(res.response.allExitingUser[i]);
          }
        }
  ...
  searchByExpectedValue(event: any) {
    if (event.target.value.length >= 3) {
      this.expectedValue = event.target.value;
      this.getAllOpportunity();
    }
    if (event.target.value.length == 0) {
      this.expectedValue = "";
